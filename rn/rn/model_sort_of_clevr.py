...
def build_mlp_model(images, questions, answers):
    ...
    # NOTE: put some descriptions into checkpoints
    tf.constant('mlp on sort-of-clevr')

    # NOTE: arXiv:1706.01427v1, supplementary material, d
    #       in this task our model used: four convolutional layers with 32, 64,
    #       128, 256 kernels, ReLU non-linearities, and batch normalization.
    #
    #       we also trained a comparable MLP based model (CNN_MLP model) on the
    #       sort-of-clevr task, to explore the extent to which a standard model
    #       can learn to answer relational questions. we used the same CNN and
    #       LSTM(???), trained end-to-end, as described above.
    tensors = images
    ...
    # NOTE: arXiv:1706.01427v1, supplementary material, d
    #       however, this time we replaced the RN with an MLP with the same
    #       number of layers and number of units per layer.
    tensors = tf.reshape(tensors, [-1, 64, 256 + 2])

    # NOTE: split so that each input has 64 features
    #       -> list([N, 1, 258], ..., [N, 1, 258])
    object_list = tf.split(tensors, 64, axis=1)

    # NOTE: reshape questions so they can be concat with object pairs
    #       -> [N, -1, 11]
    questions_temp = tf.reshape(questions, [-1, 1, 11])

    # NOTE: concat object & questions
    object_list = [tf.concat([o, questions_temp], axis=-1) for o in object_list]

    # NOTE: concat back to [N, 64, 258+11]
    tensors = tf.concat(object_list, axis=1)
